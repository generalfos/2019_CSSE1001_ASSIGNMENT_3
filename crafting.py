...
from constants import FURNACE_RECIPES
...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        # Create Flame for Smelting
        if self._input_size == (3, 1):
            flame_img = tk.PhotoImage(file='Fire.gif')
            self._flame = tk.Label(self, image=flame_img)
            self._flame.image = flame_img
            self._flame.pack(side=tk.LEFT)

        self._input = ItemGridView(self, input_size)
        self._input.pack(side=tk.LEFT)
        self._button = tk.Button(self, text='=> craft =>')
        self._button.pack(side=tk.LEFT)
        self._output = ItemGridView(self, (1, 1))
    ...
    def render(self, key_stack_pairs, selected):
        ...
        """
        # Handle normal crafting
        if self._input_size != (3, 1):

            for key, stack in key_stack_pairs:
                if key == "output":
                    self._output.draw_cell((0, 0), stack, key == selected)
                    pass
                else:
                    self._input.draw_cell(key, stack, key == selected)
                    pass
            ...
            for key, stack in key_stack_pairs:
                if key == "output":
                    ...
                    if key != (1, 0):
                        ...
                        # grid_x, grid_y = self._input.grid_to_xy_centre((1, 0))
                        # self._flame.place(in_=self._input, height=40, width=40, x=grid_x, y=grid_y)
                        pass
    ...
    def bind_for_id(self, event, callback):
        ...
        #
        # ...
        self._input.bind(event, lambda e: callback(self._input.xy_to_grid((e.x, e.y)), e))
